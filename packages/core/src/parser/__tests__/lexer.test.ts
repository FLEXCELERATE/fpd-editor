...
describe('Lexer', () => {
    ...
    it('tokenizes a complete FPD as correct token sequence', () => {
        ...
        // Verify the token stream has the expected structure
        expect(types[0]).toBe(TokenType.START_FPD);
        expect(types).toContain(TokenType.KEYWORD); // title, system, product, process_operator
        expect(types).toContain(TokenType.STRING); // "My Process", "Sys", "Raw Material"
        expect(types).toContain(TokenType.LBRACE);
        expect(types).toContain(TokenType.RBRACE);
        expect(types).toContain(TokenType.IDENTIFIER); // p1, po1, tr1
        expect(types).toContain(TokenType.FLOW); // -->
        expect(types).toContain(TokenType.USAGE); // <..>
        expect(types[types.length - 1]).toBe(TokenType.EOF);

        // Verify all string values were extracted correctly
        const strings = tokens.filter((t) => t.type === TokenType.STRING).map((t) => t.value);
        expect(strings).toContain('My Process');
        expect(strings).toContain('Sys');
        expect(strings).toContain('Raw Material');
    });

    it('handles large input without errors', () => {
        // Generate a large but valid token stream
        const lines = ['@startfpd'];
        for (let i = 0; i < 500; i++) {
            lines.push(`product p${i} "Product ${i}"`);
        }
        lines.push('@endfpd');
        ...
        const keywords = tokens.filter(
            (t) => t.type === TokenType.KEYWORD && t.value === 'product',
        );
        expect(keywords).toHaveLength(500);

...
export class FpdDiagnosticsProvider {
    ...
    public validateDocument(document: vscode.TextDocument): void {
        ...
        const declaredElements = new Map<
            string,
            { type: string; line: number; systemId: string | null }
        >();
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (elementMatch) {
                ...
                if (declaredElements.has(elementId)) {
                    ...
                    declaredElements.set(elementId, {
                        type: elementType,
                        line: lineNumber,
            ...
            if (connectionMatch) {
                ...
                if (declaredElements.has(sourceId) && declaredElements.has(targetId)) {
                    ...
                    const validationError = this.validateConnection(
                        sourceEl.type,
                        targetEl.type,
                        operator,
                        isCrossSystem,
            ...
            const malformedElementMatch = line.match(
                /^(product|energy|information|process_operator|technical_resource)\s+/,
            );

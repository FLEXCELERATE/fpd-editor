...
describe('useViewport', () => {
    ...
    describe('zoomToFit', () => {
        ...
        it('should reset to initial viewport for zero-size bounds', () => {
            ...
            act(() => {
                result.current.zoomToFit({ x: 0, y: 0, width: 0, height: 0 }, 800, 600);
            });
    ...
    describe('zoom step accumulation', () => {
        it('zoomIn steps by exactly 0.1 per call', () => {
            const { result } = renderHook(() => useViewport());
            ...
            expect(result.current.viewport.zoom).toBeCloseTo(1.3, 5);
        });

        it('zoomOut steps by exactly 0.1 per call', () => {
            const { result } = renderHook(() => useViewport());
            ...
            expect(result.current.viewport.zoom).toBeCloseTo(0.8, 5);
        });

        it('repeated zoomOut stops at minimum (10%)', () => {
            const { result } = renderHook(() => useViewport());

            // Zoom out from 100% to below 10% — should clamp
            act(() => {
                for (let i = 0; i < 20; i++) {
                    result.current.zoomOut();
                }
        ...
        it('repeated zoomIn stops at maximum (400%)', () => {
            const { result } = renderHook(() => useViewport());

            // Zoom in from 100% to above 400% — should clamp
            act(() => {
                for (let i = 0; i < 50; i++) {
                    result.current.zoomIn();
                }
    ...
    describe('handleWheel', () => {
        it('exposes handleWheel as a function', () => {
            const { result } = renderHook(() => useViewport());

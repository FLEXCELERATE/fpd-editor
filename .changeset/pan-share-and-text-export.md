...
- **Text export data loss.** With a `system { }` block present, `exportText` only wrote elements whose `systemId` matched a system — plus cross-system *flows*. States, operators, resources and usages belonging to no system were dropped, and re-reading the file then dropped the flows referencing them too. A document in that shape was destroyed by saving it. Elements outside any system are now written at the top level.
